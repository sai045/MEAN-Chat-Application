...
function router() {
  ...
  authRouter.post("/signUp", function (req, res) {
    ...
    Userdata.count({ username: userData.username }).then((count) => {
      if (count > 0) {
        console.log("Username exists.");
        res.sendStatus(404);
      } else {
        console.log("Username does not exist.");
        Userdata.count({ email: userData.email }).then((count) => {
          if (count > 0) {
  ...
  authRouter.get("/getUser/:id", function (req, res) {
    ...
    Userdata.findOne({ _id: id }).then((user) => {
      res.send(user);
    });
  });
  ...
  authRouter.post("/searchUser", function (req, res) {
    ...
    Userdata.find({ username: { $regex: `${username}`, $options: "i" } }).then(
      (users) => {
